use alloy::{providers::Provider, primitives::Address, sol};
use regex::Regex;
...
pub async fn is_erc721_nft_contract(provider: &impl Provider, addr: Address) -> bool {
    ...
    erc165.supportsInterface([0x80, 0xac, 0x58, 0xcd].into()).call().await.unwrap_or(false)
}

pub async fn is_erc1155_nft_contract(provider: &impl Provider, addr: Address) -> bool {
    let erc165 = IERC165::new(addr, provider);
    erc165.supportsInterface([0xd9, 0xb6, 0x7a, 0x26].into()).call().await.unwrap_or(false)
}
